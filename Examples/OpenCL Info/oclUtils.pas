...
function oclGetDev(cxGPUContext: Pcl_context; nr: TCL_uint): PCL_device_id;//tested +fixed = complete
...
function oclGetDev(cxGPUContext: Pcl_context; nr: TCL_uint): PCL_device_id;
var
  szParmDataBytes: TSize_t;
  cdDevices: array of PCL_device_id;
begin
  ...
  if( szParmDataBytes / SizeOf(Pcl_device_id) < nr ) then
  begin
...
begin
  ...
  //Writeln(Integer(device));
  if( _type and CL_DEVICE_TYPE_CPU )<>0 then
  begin
  ...
  if( _type and CL_DEVICE_TYPE_GPU )<>0 then
  begin
  ...
  {$ENDIF}
      ...
      //shrLog(iLogMode, 0.0, "  CL_DEVICE_TYPE:\t\t\t%s\n", "CL_DEVICE_TYPE_GPU");
  if( _type and CL_DEVICE_TYPE_ACCELERATOR )<>0 then
  begin
  ...
  if( _type and CL_DEVICE_TYPE_DEFAULT )<>0 then
  begin
  ...
  if error_correction_support=CL_TRUE then
  begin
  ...
  if( queue_properties and CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE )<>0 then
  begin
  ...
  if( queue_properties and CL_QUEUE_PROFILING_ENABLE )<>0 then
  begin
  ...
  //Writeln(Integer(device));
  if(nv_device_attibute_query)then
  begin
      ...
      if integrated_memory=CL_TRUE then
      begin
...
begin
  ...
  inc(current_device);
	...
	while( current_device < device_count )do
	begin
    // CL_DEVICE_MAX_COMPUTE_UNITS
    clGetDeviceInfo(cdDevices[current_device], CL_DEVICE_MAX_COMPUTE_UNITS, SizeOf(compute_units), @compute_units, nil);
